...
**User preferences (Jakub):** Minimalist communication ("Easy Mode"). Strict TypeScript (no `any`). Agent communication exclusively via TODO_NEXT and RELEASE_NOTES. Current project phase as of 2026-04-22: **v11 complete** (Trips UX — persist AI summary verifikace + delete trip s `confirmDeleteId` state, kaskádové smazání waypoints). Fáze 8 roadmapy uzavřena. Mutational pattern: SWR hooks are pure getters, mutations stay in `page.tsx` — binding for all future hooks.

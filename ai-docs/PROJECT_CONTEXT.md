...
- **`components/AdminPanel.tsx`:** Client Component (≤80 ř.), čistý `useState`. Tři tlačítka: **Sync Vrcholy** (`POST /api/sync-peaks`), **Sync Výzvy** (`POST /api/sync-challenges`), **Sync Oblasti** (`POST /api/sync-areas`). Per-button stav: `idle | loading | success | error`.
...
- `hooks/useMapEffects.ts` (210 ř.), `hooks/useDataFetching.ts` (228 ř.), `hooks/useChallenges.ts` (124 ř.) — kompoziční hooks s více efekty/operacemi, architektonicky odlišná kategorie od state/getter hooks. Akceptováno Architektem ve Verzi 21.
...
### Frontendová architektura (od Verze 21, aktualizováno v11)
`app/page.tsx` (~248 ř.) je čistý orchestrátor. Logika je rozdělena do:
- **Hooks:** `hooks/useMapEffects.ts`, `hooks/useDataFetching.ts`, `hooks/useChallenges.ts`, `hooks/useUserAscents.ts`, `hooks/useUserVisits.ts`, `hooks/useTrips.ts`, `hooks/useTripLayer.ts`, `hooks/useTripWaypoints.ts`, `hooks/useHoryCredentials.ts`, `hooks/useUserChallenges.ts`, `hooks/useAreas.ts`
- **Komponenty:** `MapContainer`, `ChatPanel`, `LoginScreen`, `HelpHint`, `FilterSection`, `UserSettingsPanel`, `PeaksSidebar`, `RoutesSidebar`, `ChallengesContent`, `PeakDetail`, `AuthModal`
- **Lib:** `lib/page-types.ts` (zbytek ≤13 ř.), `lib/page-utils.ts` (zbytek ≤19 ř.), `lib/page-config.ts`, `lib/map/constants.ts`, `lib/map/leaflet-loader.ts`, `lib/peaks/types.ts`, `lib/challenges/types.ts`, `lib/trips/types.ts`, `lib/czech/alphabet.ts`, `lib/czech/osmismerka.ts`
...
* Spouští aktualizační cykly: **Oblasti**, **Vrcholy**, **Výzvy**, **Zámky** — přes dedikovaná tlačítka v admin UI.
* Adminovské credentials pro hory.app jsou uloženy **šifrovaně v DB** (`data_sources.config`, JSONB, klíč `SETTINGS_ENCRYPTION_KEY`) — zadají se jednou přes formulář `HoryCredentialsForm` v AdminPanelu. Odděleny od uživatelských credentials.
* Admin role je určena sloupcem `role` v tabulce `user` (hodnota `'admin'`), nikoli env proměnnou.
...
|---|---|---|
| Admin credentials (hory.app) | `data_sources.config` (šifrovaný JSONB v DB) | Seed/sync DB — zadá admin jednou přes AdminPanel formulář |
| User credentials (hory.app) | `user_module_settings` (šifrovaný JSONB) | Synchronizace osobních výstupů uživatele při přihlášení |
| Trek účet | Better Auth DB (`user.role` = `'user'`/`'admin'`) | Přihlášení do Trek aplikace, správa sessions, admin role |
...
- [x] **v15a — Unified clustering vrcholy + zámky:**
  ...
  - [x] `hooks/useCastleLayer.ts` smazán (byl no-op stub, rendering v `useMapEffects`).
...
- [x] **v16–v21 — Sidebar hook, plánování merge, floating overlays, castle filter:**
  - [x] `useModuleSidebar` hook (generický přes TypeScript generics).
  - [x] Sloučení plánování do jednoho menu (`"planovani"` + `planningTab`).
  - [x] Floating overlays pro PeakDetail + CastleDetail.
  - [x] Castle filter toggle (analogie peaks).
  - [x] `scripts/scrape-castles.ts` přidán. TD-stub `useCastleLayer.ts` smazán.
- [x] **v16-deploy — DO App Platform deploy stabilizace:**
  - [x] DB konektivita: lazy init singleton, SSL s `rejectUnauthorized: false`, strip `sslmode` z URL.
  - [x] Vlastní migrace `scripts/db-migrate.ts` (trackuje v `public._migrations`, obchází DO permission omezení).
  - [x] Admin role DB-backed: `user.role varchar(32)`, `lib/db/admin.ts`, migrace `0001_left_living_lightning.sql`.
  - [x] BetterAuth `baseURL` z env, auth-client bez `baseURL` (inferuje z `window.location.origin`).
  - [x] `CastlesScraperService` smazán (mrtvý kód), sync-castles čte z `export.geojson` / `CastlesParserService.parse()`.
- [x] **v16b-bugfix — Produkční opravy:**
  - [x] `getLocationsByModuleAndCountry()` — JOIN přes `location_types`, `/api/peaks` filtruje výhradně mountains modul.
  - [x] 504 fix: `/api/sync-peaks` vrací `202` okamžitě, Playwright běží fire-and-forget.
  - [x] AdminPanel polling `/api/peaks` každých 10s po zahájení sync.
  - [x] `selectedCastle` state čistí při přepnutí modulu (stale overlay fix).
- [x] **v16c-admin-credentials — Globální admin credentials v DB:**
  - [x] `app/api/admin/hory-credentials/route.ts` — GET/POST endpoint (šifrovaně do `data_sources.config`).
  - [x] `lib/hory-auth.ts` — `getAdminHoryCredentials()` async z DB; `resolveHoryCredentials()` zachován.
  - [x] `components/AdminPanel.tsx` — `HoryCredentialsForm` formulář.
- [x] **v16e-static-data-files — Lokální scraping, statické soubory:**
  - [x] `scripts/scrape-peaks.ts` + `scripts/scrape-areas.ts` — lokální Playwright scraper → `data/peaks.json` / `data/areas.json`.
  - [x] `/api/sync-peaks` + `/api/sync-areas` — čtou ze statických souborů (žádný Playwright na serveru).
  - [x] `package.json` — skripty `scrape:peaks` a `scrape:areas`.

---

## Technická Specifika projektu

### Admin panel (přepracován v9.4, rozšířen v16-deploy + v16c)
- **`app/(admin)/admin/page.tsx`:** Server Component. Admin check přes `isAdmin(userId)` z DB (`lib/db/admin.ts`). `ADMIN_EMAILS` env odstraněn. Admini vidí `AdminPanel` + `HoryCredentialsForm`. Přidán odkaz "← Zpět na mapu".
- **`components/AdminPanel.tsx`:** Client Component. Pět oblastí: **Sync Vrcholy** (`POST /api/sync-peaks`, 202 fire-and-forget + polling), **Sync Výzvy** (`POST /api/sync-challenges`), **Sync Oblasti** (`POST /api/sync-areas`), **Sync Zámky** (`POST /api/sync-castles`), **Hory.app Credentials** (`HoryCredentialsForm` — GET/POST přes `/api/admin/hory-credentials`). Per-button stav: `idle | loading | success | error`.
...
- **`app/api/admin/hory-credentials/route.ts`:** GET/POST — čte/zapisuje globální Hory.app admin credentials do `data_sources.config` (JSONB, šifrovaně), admin-only.
- **`lib/hory-auth.ts`:** `getAdminHoryCredentials()` — async, čte z DB. `resolveHoryCredentials()` — explicitní předání credentials (legacy, zachováno).
...
- **Datový tok vrcholů (od v16e):** Lokálně: `pnpm scrape:peaks` (`scripts/scrape-peaks.ts`, Playwright) → `data/peaks.json` → commit & push. Produkce: Admin panel → `POST /api/sync-peaks` → čte `data/peaks.json` → upsert do DB → `GET /api/peaks`. Žádný Playwright na serveru. Stejný vzor: `pnpm scrape:areas` → `data/areas.json`. **`/api/sync-peaks` — request.json() fallback (opraveno v9.4.3):** `AdminPanel` posílá POST bez těla; `request.json()` se zachytí přes `.catch(() => ({}))` — body je volitelné.
...
- `data_sources`: `id`, `module_id` (FK), `config` (JSONB: `targetUrl`, `countryCode`, `crawlRanges`). **Nové využití (v16c):** Globální admin Hory.app credentials uloženy do `data_sources.config` (modul `mountains`, type `scraper`) — šifrovaně přes `lib/crypto.ts`.
- **`user` tabulka (Better Auth, rozšířena v16-deploy):** Sloupec `role varchar(32) DEFAULT 'user'`. Admin se nastaví ručně: `UPDATE "user" SET role = 'admin' WHERE email = '...'`. `lib/db/admin.ts` — `isAdmin(userId)` dotaz do DB. Migrace: `drizzle/0001_left_living_lightning.sql`.
...
- **`GET /api/castles`** (veřejný) + **`POST /api/sync-castles`** (admin-only, `isAdmin(userId)` z DB).
...
- **Import dat (od v16-deploy + v22 příprava):** `CastlesScraperService.ts` smazán (v16-deploy — mrtvý kód). Sync-castles čte z lokálního `data/castles.geojson` přes `CastlesParserService.parse()`. `scripts/scrape-castles.ts` přidán (v16–v21) — lokální scraper bez Playwright (Overpass API), výstup do `data/castles.geojson`. Soubor ještě není commitnutý — **blokuje produkci** (Priorita 1, v22). `export.geojson` v kořeni repozitáře není potřeba — parser hledá `data/castles.geojson` jako první.
...
#### Migrace & DB konektivita (od v16-deploy)
- **`scripts/db-migrate.ts`:** Vlastní migrační skript (nahradil Drizzle ORM migrátor). Trackuje migrace v `public._migrations` (DO Managed DB uživatel nemá práva vytvářet `drizzle` schéma). Idempotentní — zachytí PostgreSQL chyby `42701` (duplicate_column), `42P07` (duplicate_table), `42710` (duplicate_object).
- **`lib/db/lazy-init.ts`:** Singleton `ensureDbInitialized()`. Seed se spustí při prvním HTTP requestu (ne při startu) — kvůli DO private network DNS (hostname `base` se resolvuje až po registraci kontejneru do service mesh).
- **`lib/db/index.ts`:** Strip `sslmode` z URL + `ssl: { rejectUnauthorized: false }` + `NODE_TLS_REJECT_UNAUTHORIZED = "0"` — pro DO Managed Database se self-signed certifikátem.
- **Migrace na produkci:** `pnpm db:migrate` v DO App Console → `UPDATE "user" SET role = 'admin' WHERE email = '...'` → první request na `/api/castles` spustí seed.
